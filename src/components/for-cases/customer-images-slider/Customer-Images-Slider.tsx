...
function preloadImages(srcArray: string[]): Promise<void> {
    return Promise.all(
        srcArray.map(
            (src) =>
...
export default function HorizontalScrollSlider({ images }: Props) {
    ...
    useEffect(() => {
        ...
        const fullImagePaths = images.map((img) => `/${img}`);

        // Предзагружаем все изображения перед запуском анимаций
        preloadImages(fullImagePaths).then(() => {
                ...
                >
                    <div className={`flex justify-center h-[80vh] ${i % 2 == 0 ? 'items-start' : 'items-end'}`}>
                        <img
                            ...
                            loading="eager" // Загружаем изображения сразу
                            style={{ willChange: 'transform, opacity' }} // Для оптимизации производительности
                        />
                    </div>
                </div>
            ))}
